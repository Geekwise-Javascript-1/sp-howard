  ...
  addCheckBoxListener : function() {
    ...
    for (var i = 0; i < checkBox.length; i++) {
      if (checkBox[i].classList.contains('unchecked')) {
        checkBox[i].addEventListener('change', toDoList.completed);
        toDoItem[i].addEventListener('click', toDoList.edit);
      } else {
        checkBox[i].addEventListener('change', toDoList.undo);
      }
    }
  },
  completed : function() {
...
addBtn.onclick = toDoList.addToList;
